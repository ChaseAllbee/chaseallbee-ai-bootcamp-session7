...
The todo-service is a baseline full-stack task management app used for the platform golden path: a React frontend calling a stateless Node.js/Express CRUD API (`/api/todos`), currently backed by in-memory storage for lab simplicity. In this lab, the service is containerized and hosted on AWS ECS Fargate behind an Application Load Balancer, with infrastructure managed by Terraform and operational telemetry collected in CloudWatch (including Container Insights).
